...
{   
    //Variables
    public List<GameObject> mainAlphabet; //List of letters GameObjects present on knobs
    public List<GameObject> sideAlphabet; //List of letters GameObjects present on side knobs
    ...
    private bool mainGlow = true; //flag to alternate between glowing and regular text
    private bool sideGlow = true;
    ...
    void Update()
    {
        ...
        if (timer > 2.0f)
        {
            ToggleGlow(mainAlphabet);
            //ToggleGlow(sideAlphabet);
            timer = 0.0f;
        }
    }
    ...
    This function is called every 2 seconds, and it alternates the glowing by checking the mainGlow flag, which is flipped 
    each time function is called.  
     */
    void ToggleGlow(List<GameObject> Alphabet)
    {
        foreach (GameObject letter in Alphabet)
        {
            ...
            /*
            If the mainGlow flag is set, it means the outer knob should glow. It checks the OuterText flag (which is only ever set to true for the outer knob),
            ...
            */
            ...
            /*
            If the mainGlow flag is not set, then it means the inner knob should glow. The exact same behaviour as above repeats, 
    ...
    void ResetMaterials()
    {
        foreach (GameObject letter in mainAlphabet)
        {
